...
Step 3 **creates** `O` / `H` / `A` nodes and **edits** each processed curated source node (adds the `extracted` list and body `![[embeds]]`; the source's `data_basis` was set at step 2). It mints **no judgment numbers** — data-reliability is inherited from the source, and validity / likelihood / prior are later steps. Mint every id'd node (`O` / `H` / `A` / `D`) with `python3 .claude/skills/flf-epistack/scripts/create_node.py <analysis-dir> --type <type> --title "..."`, never a hand-picked id — the script takes the next free `PREFIX-N` under a lock, so parallel children never collide (one counter per prefix, gaps are fine).
