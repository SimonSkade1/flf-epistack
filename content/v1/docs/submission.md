...
**The trust floor can silence the side most worth hearing.** `combined_score = usefulness × (trust_score − baseline)`, baseline 0.8, cuts everything below baseline trust — and a contrarian position is systematically less-published, so the cut correlates with dissent rather than error. Bounded by a logged exception in step 2's cut rules: the single strongest source for an otherwise-unrepresented position may be curated even at `combined_score ≤ 0`, its unchanged low `trust_score` still discounting it in every downstream likelihood, the departure logged in `curation_reason` and `agent-notes/curation.md`. Live instance: Plaga's metastable-black-hole preprint scores `trust_score: 0.45`, `usefulness: 4.0`, `combined_score: −1.4` — under the floor, curated anyway, its hypotheses (H-50, H-51, H-52) extracted as first-class candidates that will compete as ordinary cluster members. It still costs: the exception admits *one* source per position, by judgment, not by rule.
...
**The anthropic shadow is unsolved.** Survival-based safety bounds — Earth, Sun and white dwarfs still here after gigayears of cosmic-ray bombardment — are observation-selected: we could not find ourselves in the branch where the mechanism fired. The pipeline surfaced this about itself: observation O-91 records that the RHIC cosmic-ray bounds omitted observer selection for roughly five years until Tegmark and Bostrom's 2005 correction, which restored a bound only by adding anthropic reasoning plus a galactic planet-formation model. But the machinery still treats survival as ordinary evidence; the correction enters as an argument *about* the observation, not as a schema feature, and nothing forces it to be made.
